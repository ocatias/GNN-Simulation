...
class CellularRingEncoding(BaseTransform):
    ...
    def encode_with_id_maps(self, data: Data):  
        ...
        len_e_feat = 0
        if has_edge_attr:
            edge_attr = torch.add(edge_attr, 1)
            ...
            e_identifiers = torch.zeros([edge_attr.shape[0], 1])
            edge_attr = torch.cat((e_identifiers, edge_attr), dim = 1)
        ...
        for i in range(edge_index.shape[1]):
            ...
            if has_edge_attr:
                ...
                if self.aggr_edge_atr:
                    print(torch.tensor([1]).shape, edge_attr[i,start_e_feat_on_e:].shape)
                    new_e_feat = torch.cat([torch.tensor([1]), edge_attr[i,start_e_feat_on_e:]]) 
                    
                    print(edge_attr.shape, new_e_feat.shape)
                    ...
                    #  Add features (encoding edge type) to edge_attr so we have an edge_attr for every edge
                    edge_attr = torch.cat((edge_attr, torch.ones([4, len_e_feat])), 0)
        ...
        for ring in rings:
            ...
            if x is not None:
                ...
                if self.aggr_vertex_feat and x is not None:
                    ...
                    for vertex in ring:
                        new_v_feature += x[id_maps[0][(vertex,)], start_v_feat:start_e_feat]

                    new_v_feature = new_v_feature / len(ring)
            ...
            for idx in range(len(ring) - 1):
                ...
                new_e_feat = torch.cat([torch.tensor([3]), torch.zeros([len_e_feat])])
                edge_attr = torch.cat((edge_attr, torch.stack([new_e_feat, new_e_feat])), 0)

            
            # Add to features
            if self.edge_attr_in_vertices and x is not None:
                new_aggr_edge_features_for_vertex = new_aggr_edge_features_for_vertex / (len(ring)-1)
                # new_v_feature = torch.cat((new_v_feature, new_aggr_edge_features_for_vertex), 0)
                new_v_feature = torch.zeros_like(x[0,:])
                new_v_feature[2] = 1
                ...
                for idx2 in range(len(edge_vertices)):
                    ...
                    if (p,q) in newly_created_edges or (q,p) in newly_created_edges:
                        ...
                        if self.aggr_edge_atr:
                            ...
                            lower_dim_edge_feat = torch.cat([torch.tensor([2]), torch.zeros([len_e_feat])])
