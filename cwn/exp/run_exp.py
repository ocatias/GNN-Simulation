...
def main(args):
    ...
    if args.dataset in ["ZINC", "MOLLIPO", "MOLESOL"]:
        mode = "min"
    else:
        mode = "max"

    map_list_to_float = lambda ls: list(map(lambda x: float(x), ls))
    return {
        "best_epoch": int(best_val_epoch + 1),
        "epochs": int(epoch),
        "final_val_results": float(valid_curve[best_val_epoch]),
        "final_test_result": float(test_curve[best_val_epoch]),
        "final_train_loss": float(train_loss_curve[best_val_epoch]),
        "parameters": trainable_params,
        "mode": mode,
        "loss_train": map_list_to_float(train_loss_curve), 
        "runtime_hours":  runtime_hours,
        "result_val": map_list_to_float(valid_curve),
        "result_test": map_list_to_float(test_curve)
    } 
