...
def convert_graph_dataset_with_rings(dataset, max_ring_size=7, include_down_adj=False,
    ...
    for c, complex in enumerate(complexes):
        ...
        if complex.y is None:
            ...
            graph_is_labeled = graph.y == graph.y
            complex_is_labeled = complex.y == complex.y
            assert torch.equal(complex.y[graph_is_labeled], graph.y[graph_is_labeled])
            assert torch.equal(complex.y[complex_is_labeled], graph.y[complex_is_labeled])
            assert torch.equal(graph_is_labeled, complex_is_labeled)
