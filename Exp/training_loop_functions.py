...
def compute_loss_predictions(batch, model, metric, device, loss_fn, tracking_dict, classes):
    ...
    if y.shape[1] == 1 and metric == "accuracy":
        y = F.one_hot(torch.squeeze(y, 1), classes)
...
def train(model, device, train_loader, optimizer, loss_fct, eval_name, use_tracking, metric_method=None, classes=10):
    """
    ...
    for step, batch in enumerate(train_loader):
        optimizer.zero_grad()
        loss = compute_loss_predictions(batch, model, eval_name, device, loss_fct, tracking_dict, classes)
...
def eval(model, device, loader, loss_fn, eval_name, metric_method=None, classes=10):
    """
    ...
    for step, batch in enumerate(loader):
        with torch.no_grad():
            compute_loss_predictions(batch, model, eval_name, device, loss_fn, tracking_dict, classes)
